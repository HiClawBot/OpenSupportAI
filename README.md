...
- Lease-backed async jobs with atomic PostgreSQL claims, heartbeats, stale-job recovery, owner fencing, graceful worker drain, and a real `knowledge.index` handler.
...
- 基于租约的异步任务：PostgreSQL 原子领取、heartbeat、过期任务恢复、owner fencing、worker 优雅排空，以及真实的 `knowledge.index` handler。
