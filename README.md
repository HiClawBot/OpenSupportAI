...
The current `main` branch is a controlled staging candidate, not a claim of unattended production readiness. The next public maturity release is planned as `v1.1.0-beta.1`; remaining browser recovery coverage, load/fault testing, and a 24-hour staging soak still gate that prerelease.
...
当前 `main` 是可控 staging candidate，不代表已经达到无人值守生产就绪。下一条公开成熟度发布线计划为 `v1.1.0-beta.1`；剩余浏览器恢复覆盖、负载/故障测试和 24 小时 staging soak 仍是该 prerelease 的发布门禁。
