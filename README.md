...
<p align="center">
  <strong>Open-source, embeddable, LLM-native AI support runtime.</strong>
</p>
...
The current `main` branch is a controlled staging candidate, not a claim of unattended production readiness. The next public maturity release is planned as `v1.1.0-beta.1`; a release-qualifying 24-hour staging soak and final provenance artifacts still gate that prerelease.
...
- Bounded project-scoped PostgreSQL lexical retrieval with indexed-document filtering, English full-text search, CJK n-grams, trigram fallback, deterministic thresholds, and stable source citations.
- OpenAI-compatible LLM and embedding client package.
- Headless JavaScript SDK.
- Embeddable bilingual browser widget with Shadow DOM UI, SSE updates, session-expiry/network recovery actions, conversation persistence, source references, and handoff action.
...
- Persisted worker process heartbeats, migration-aware readiness, authenticated runtime/queue metrics, and correlated JSON job lifecycle logs.
...
- Versioned deterministic evaluation suites, persisted run evidence, and governed evolution proposals with human approval, regression, canary, promotion, and rollback gates.
- GitHub Actions CI with desktop/mobile browser recovery and governance coverage, plus a live Chatwoot smoke-test script for release validation.
- Docker Compose stack for PostgreSQL/pgvector, Redis, MinIO, API, worker, admin console, demo app, and optional Chatwoot.
- Minimal production Compose path with non-root API/worker images, a one-shot migration task, read-only filesystems, worker-loss detection, and CI-rehearsed PostgreSQL backup/restore.
- Reproducible bounded-load, LLM/tool outage, worker queue-recovery, and 24-hour soak probes with machine-readable evidence.
...
packages/
  evals/                  Deterministic scenario evaluator and evidence contracts
...
The production Beta topology contains only PostgreSQL/pgvector, a one-shot migration task, one API, and one worker. It does not seed demo data or require Redis/MinIO:

```bash
cp deploy/docker-compose/production.env.example .env.production
# Fill every blank secret and DATABASE_URL. URL-encode database credentials.
...
`/health/live` proves only that the API process responds. `/health/ready` returns `503` until the expected migration and a current worker heartbeat with both required job types are present. Root administrators can inspect sanitized process, queue, migration, and worker metrics at `GET /v1/admin/ops/metrics`.

Operational qualification is documented in pure English at [docs/BETA_OPERATIONS.md](./docs/BETA_OPERATIONS.md) and pure Chinese at [docs/BETA_OPERATIONS.zh-CN.md](./docs/BETA_OPERATIONS.zh-CN.md). The production Compose gate runs bounded load, LLM/tool outage, worker-down queue recovery, and a short soak-harness check. The short run never qualifies a release; `v1.1.0-beta.1` requires a report whose measured elapsed time is at least 24 hours.

```bash
API_URL=https://api.example.com ADMIN_TOKEN=... pnpm smoke:load
# Run only against a disposable project because this changes provider/tool config.
API_URL=https://api.example.com ADMIN_TOKEN=... pnpm smoke:faults
SOAK_DURATION_SECONDS=86400 SOAK_REQUIRE_24_HOURS=true \
  SOAK_REVISION=$(git rev-parse HEAD) SOAK_ENVIRONMENT=staging \
  SOAK_REPORT_PATH=output/beta-evidence/soak.json \
  API_URL=https://api.example.com ADMIN_TOKEN=... pnpm soak:beta
```
...
Reindexing creates a `knowledge.index` async job. The worker marks the document `indexing`, rebuilds all chunks from stored content, then marks it `indexed` or `failed` with an error message. Prisma-backed retrieval uses PostgreSQL FTS and trigram indexes over trigger-maintained lexical text, filters to `indexed` documents, returns a bounded candidate set, and applies the same deterministic relevance threshold as memory mode. The schema remains pgvector-ready for a later hybrid/vector pass.
...
OpenSupportAI can persist versioned evaluation suites and deterministic run evidence, then turn a failed run into a governed knowledge, prompt, or tool proposal. Every state change requires the `admin:evolution` scope and is written to the audit log.
...
A proposal cannot reuse its source run as regression evidence. Canary entry requires deployment, scope, and rollback evidence; promotion requires a passed canary outcome. The system never applies proposal artifacts to production knowledge, prompts, or tools automatically.

The critical deterministic corpus is available at `evals/golden/beta-core.v1.json`:
...
See [Governed Evaluation and Evolution](./docs/GOVERNED_EVOLUTION.md) and its [Chinese edition](./docs/GOVERNED_EVOLUTION.zh-CN.md).
...
DATABASE_URL=postgresql://... pnpm smoke:postgres-evolution
DATABASE_URL=postgresql://... pnpm smoke:postgres-retrieval
pnpm --filter @opensupportai/site dev
...
当前 `main` 是可控 staging candidate，不代表已经达到无人值守生产就绪。下一条公开成熟度发布线计划为 `v1.1.0-beta.1`；满足发布条件的 24 小时 staging soak 和最终来源证明产物仍是该 prerelease 的发布门禁。
...
- 有界、项目隔离的 PostgreSQL 词法检索，包含 indexed 文档过滤、英文全文检索、中文 n-gram、trigram fallback、确定性阈值和稳定来源引用。
- OpenAI-compatible LLM 与 embedding 客户端包。
- Headless JavaScript SDK。
- 可嵌入中英文浏览器 Widget：Shadow DOM UI、SSE 更新、会话过期/网络错误恢复、会话持久化、source references 和人工转接。
...
- 持久化 worker 进程心跳、migration-aware readiness、需要 root 鉴权的运行时/队列指标，以及带关联字段的 JSON job 生命周期日志。
...
- 版本化确定性评测套件、持久化运行证据，以及带人工审批、回归、灰度、晋级和回滚门禁的治理进化提案。
- GitHub Actions CI 包含桌面/移动端浏览器恢复与治理流程覆盖，并提供真实 Chatwoot smoke-test 脚本。
- Docker Compose：PostgreSQL/pgvector、Redis、MinIO、API、worker、admin console、demo app，以及可选 Chatwoot。
- 最小生产 Compose 路径：非 root API/worker 镜像、一次性 migration task、只读文件系统、worker 失联检测，以及由 CI 演练的 PostgreSQL 备份恢复。
- 可复现的有界负载、LLM/工具故障、Worker 队列恢复和 24 小时 soak 探针，并输出机器可读证据。
...
生产 Beta 拓扑只包含 PostgreSQL/pgvector、一次性 migration task、一个 API 和一个 worker，不会 seed demo 数据，也不依赖 Redis/MinIO：

```bash
cp deploy/docker-compose/production.env.example .env.production
# 填写全部空白 secret 和 DATABASE_URL；数据库凭据需要进行 URL 编码。
docker compose --env-file .env.production \
  -f deploy/docker-compose/docker-compose.production.yml up -d --build
curl http://127.0.0.1:4000/health/live
curl http://127.0.0.1:4000/health/ready
...
`/health/live` 只证明 API 进程能够响应。只有预期 migration 已完成，并且存在同时监听两个必需 job 类型的有效 worker 心跳时，`/health/ready` 才返回 `200`，否则返回 `503`。公开 readiness 只返回组件状态和原因码；Root 管理员可通过 `GET /v1/admin/ops/metrics` 查看已脱敏的进程、队列、migration 和 worker 详细指标。

运维验收的纯英文说明见 [docs/BETA_OPERATIONS.md](./docs/BETA_OPERATIONS.md)，纯中文说明见 [docs/BETA_OPERATIONS.zh-CN.md](./docs/BETA_OPERATIONS.zh-CN.md)。生产 Compose 门禁会执行有界负载、LLM/工具故障、Worker 停机期间队列恢复和短时 soak 工具检查。短时运行永远不能作为发布合格证据；`v1.1.0-beta.1` 必须提供实际运行时间不少于 24 小时的报告。

```bash
API_URL=https://api.example.com ADMIN_TOKEN=... pnpm smoke:load
# 该命令会修改 provider/tool 配置，只能用于一次性项目。
API_URL=https://api.example.com ADMIN_TOKEN=... pnpm smoke:faults
SOAK_DURATION_SECONDS=86400 SOAK_REQUIRE_24_HOURS=true \
  SOAK_REVISION=$(git rev-parse HEAD) SOAK_ENVIRONMENT=staging \
  SOAK_REPORT_PATH=output/beta-evidence/soak.json \
...
Reindex 会创建 `knowledge.index` async job。Worker 会把文档标记为 `indexing`，用已保存的原文重建全部 chunks，然后将文档标记为 `indexed` 或带错误信息的 `failed`。Prisma retrieval 使用 PostgreSQL FTS 与 trigram 索引查询 trigger 维护的 lexical text，只读取 `indexed` 文档，返回有界候选，并与 memory mode 共用确定性相关性阈值；schema 继续保留 pgvector-ready 字段，供后续 hybrid/vector retrieval 使用。
...
OpenSupportAI 可以持久化版本化评测套件和确定性运行证据，并将失败运行转化为知识、提示词或工具提案。每次状态变更都要求 `admin:evolution` 权限并写入审计日志。
...
提案不能复用源运行作为回归证据。进入灰度必须提供部署、范围和回滚证据；晋级必须提供通过的灰度结果。系统不会自动把提案内容写入生产知识库、提示词或工具。

关键确定性语料位于 `evals/golden/beta-core.v1.json`：
...
完整说明见[治理评测与进化](./docs/GOVERNED_EVOLUTION.zh-CN.md)及其[英文版](./docs/GOVERNED_EVOLUTION.md)。
