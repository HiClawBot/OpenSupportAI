...
The v0.4 release line is still intentionally small: it is not a full CRM or ticketing suite, and it does not copy Chatwoot, Tiledesk, or Zammad. Instead, OpenSupportAI provides the AI support runtime that can hand conversations to those systems.
...
v0.4 版本线仍然保持小而清晰：不做完整 CRM，不做复杂工单系统，也不复制 Chatwoot、Tiledesk 或 Zammad。OpenSupportAI 专注于 AI 客服运行时，并可以把会话转交给这些客服系统。
