...
- Multi-channel adapter foundation with a tested generic webhook inbound adapter, admin adapter catalog/test APIs, and Slack/email/Telegram contract stubs.
...
packages/
  ...
  adapters/channels/      Generic webhook and channel adapter contracts
...
- 多渠道 adapter 基础：可测试的 generic webhook 入站 adapter、管理端 adapter catalog/test API，以及 Slack/email/Telegram 契约 stub。
...
packages/
  ...
  rag/                    文本切块与检索工具
  adapters/channels/      Generic webhook 与 channel adapter 契约
