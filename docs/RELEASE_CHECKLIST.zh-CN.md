...
`production-compose-smoke.sh` 需要 Docker Compose；GitHub Actions 会自动执行。它是生产 Beta 的镜像、migration、readiness、有界并发与幂等、LLM/工具故障回退、worker 停机期间任务接收与恢复、短时 soak 工具、PostgreSQL 备份恢复门槛。本机没有 Docker 时不能把静态 YAML 检查替代为该项通过。

## 24 小时 Soak 门禁

短时 CI 运行只验证探针本身，不具备发布资格。候选版本必须在 staging 上执行：
...
- `qualified_for_beta=true`。
- 实际 `elapsed_seconds>=86400`。
- `provenance.revision` 与被测候选版本的 Git revision 一致。
- 默认至少完成 1368 个周期，失败数、重复消息和 readiness failure 均为 0。
- 消息接收 p95、回答完成 p95 和 API RSS 增长均未超过报告内记录的阈值。

原始报告不提交到源码仓库；应作为 GitHub prerelease 资产保存。详细说明见 `docs/BETA_OPERATIONS.zh-CN.md`。
